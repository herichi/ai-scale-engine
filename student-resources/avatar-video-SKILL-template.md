...
description: |
  ...
  and using your cloned voice instead of the robotic Seedance default.
  Use when: generating any clip of your avatar speaking a script (welcome
  videos, hooks, funnel/ad scripts, community content, any "make a video
  of me/my avatar saying..." request in this project). Enforces consistent
  avatar identity, studio, lighting, camera, and speech pace across clips
  so a multi-part series reads as one continuous recording, and burns
  captions onto the final assembled output.
...
allowed-tools: mcp__<HIGGSFIELD_MCP_ID>__generate_video, mcp__<HIGGSFIELD_MCP_ID>__voice_change, mcp__<HIGGSFIELD_MCP_ID>__explainer_video, mcp__<HIGGSFIELD_MCP_ID>__job_display, Read, AskUserQuestion
---

# Avatar video generation — AI Scale Engine template

Generates one Seedance clip of YOUR locked avatar, per your project's
locked creative and technical bible, then produces a captioned final cut.

## Setup — fill these in before first use

Before this skill works for you, do two things:
...
2. **Create your own three source-of-truth files** inside your project
   folder (copy the structure from this template's sibling
   ...
   - `visual-reference.md` — your locked hero image and its generation
     settings.
   - `AVATAR_LOCK.md` — your Higgsfield Element ID for your avatar identity,
     and your cloned voice ID. **Fill in the two placeholders below with
     your own real IDs before running this skill:**
     - `YOUR_AVATAR_ELEMENT_ID` — your `show_reference_elements` Element id
     - `YOUR_VOICE_ID` — your `create_voice`/`create_voice_from_confirmed_audio`
       voice id
...
1. Read your `GLOBAL_VIDEO_DIRECTION.md`, `visual-reference.md`, and
   `AVATAR_LOCK.md` to pull your locked Element ID, hero image job id,
   studio/lighting description, and all standing rules.
2. Compute target duration from script word count ÷ words-per-second. If
   duration would exceed 15s, stop and tell the user the split points
   needed — do not generate a truncated clip.
...
4. Call `generate_video` with `model: seedance_2_0`, `aspect_ratio: 9:16`
   ...
   media role, and `declined_preset_id` set if a preset match notice
   appears on a preflight (if you always want literal generation, never a
   stylized preset).
5. Preflight cost with `get_cost: true` first and confirm with the user
   ...
   paid generation.
6. Once the video job completes, run `voice_change` with your own
   `voice_id` and `voice_type: element` to replace the native Seedance
   voice with your cloned voice. This step is NOT optional if you've
   cloned your voice — the native voice is robotic and not yours.
7. `voice_change` jobs commonly sit in `waiting` status for 10+ minutes
   before moving to `processing`/`completed`. This is normal — do not
   retry before ~10 minutes have passed, and only retry once if still
   `waiting` after that (retrying too early risks duplicate jobs and wasted
   credits).
8. **Captions (if your GLOBAL_VIDEO_DIRECTION.md requires them):** once
   the voice-swapped clip is ready — and if this is a multi-clip series,
   once all clips for this piece are ready — run `explainer_video` to
   assemble/stitch the final cut with `subtitles: {font: ...}` set. This
   transcribes the final voiceover via Whisper and burns timed captions
   onto the output automatically. Pick a font matching your brand from the
   tool's options (patrick, caveat, marker, anton).
9. Present the final clip URL to the user for approval. Do not treat a
